 ...
 * @author Luke Last
 */
public class MazeViewTemp extends JComponent implements ComponentListener, Listener<MazeCell>
{
   ...
   /**
    * Holds the active cell that the mouse is hovering over. This should only be
    * changed through its setter method.
    */
   ...
   protected boolean drawFog = false;
   protected Set<MazeCell> unexplored;
   ...
   public void setRobotPosition(Point newLocation, double newRotation)
   {
      ...
      try
      {
         if (newLocation != null)
         {
            final MazeCell cell = this.getHostMazeCell(newLocation);
            this.invalidateCell(cell);
            this.invalidateCell(cell.plusX(1));
            this.invalidateCell(cell.plusY(1));
            if (cell.getX() > 1)
               this.invalidateCell(cell.plusX(-1));
            if (cell.getY() > 1)
               this.invalidateCell(cell.plusY(-1));
   ...
   protected void paintComponent(final Graphics arg)
   {
      if (model != null)
      {
         ...
         {
            this.background = new BufferedImage(getWidth(),
                                                getHeight(),
   ...
   private void redrawAll(final Graphics2D g)
   {
      ...
      //Enabling this will turn off transparency which will look ugly but allows a performance comparison.
      ...
      if (this.editable == false && this.robotLocation != null && false)
      {
   ...
   private Rectangle getWallAreaHalf(MazeCell cell, Direction wall)
   {
      final Rectangle area = this.getWallLocation(cell, wall);
      switch (wall)
      {
         case North :
            area.height /= 2;
   ...
   /**
    * The active cell just gets a special mouse hover effect.
    * @param cell The cell to set as the currently active cell.
    */
   private void setActiveCell(MazeCell cell)
   {
      if ( (this.active != null && cell != null && !this.active.equals(cell)) ||
          (this.active == null && cell != null) ||
          (this.active != null && cell == null))
      {
         // Invalidated the previous active cell.
         this.invalidateCell(this.active);
    ...
    */
   protected void invalidateCell(MazeCell cell)
   {
   ...
   protected void invalidateAllCells()
   {
         ...
         mouseAdapter = new MouseAdapter()
         {
            ...
            public void mouseMoved(MouseEvent e)
            {
               if (model != null)
               {
                  try
                  {
                     setActiveCell(getHostMazeCell(e.getPoint()));
                  }
                  catch (Exception ex)
                  {}
               }
   ...
   private void drawInvalidatedCells(final Graphics2D g)
   {
      synchronized (this.invalidatedCells)
      {
         for (MazeCell cell : this.invalidatedCells)
         {
            this.drawCell(g, cell);
   ...
   {
      if (cell == null || g == null || this.model == null)
      {
         return;
      }
      final Rectangle cellArea = new Rectangle(cell.getXZeroBased() * this.csm.getCellWidth(),
                                               cell.getYZeroBased() * this.csm.getCellHeight(),
                                               this.csm.getCellWidth(),
                                               this.csm.getCellHeight());
      this.paints.drawCellBackground(g, cellArea);
      if (cell.equals(this.active))
      {
         this.paints.drawCellHover(g, cellArea);
      }
      for (Direction direction : Direction.values())
      {
         
            final Rectangle area = this.getWallAreaHalf(cell, direction);
            if (this.model.getWall(cell, direction).isSet())
            {
               this.paints.drawWallSet(g, area);
            }
            else
            {
               this.paints.drawWallEmpty(g, area);
            }
         
      }
      for (PegLocation peg : PegLocation.values())
      {
         this.paints.drawPeg(g, this.getPegRegion(cell, peg));
      }
      if (this.drawFog && this.unexplored != null && this.unexplored.contains(cell))
      {
         //         this.paints.drawFog(g, new Rectangle(cellArea.x - this.csm.getWallWidthHalf(),
         //                                              cellArea.y - this.csm.getWallHeightHalf(),
         //                                              cellArea.width + this.csm.getWallWidth(),
         //                                              cellArea.height + this.csm.getWallHeight()));
         this.paints.drawFog(g, cellArea);
      }
      if (this.editable == false && this.robotLocation != null)
      {
         // Check if the robot or any part of it is in this cell.
         final int x = this.robotLocation.x;
         final int y = this.robotLocation.y;
         if (cellArea.contains(this.robotLocation) ||
             cellArea.contains(x, y - this.csm.getCellHeightHalf()) ||
             cellArea.contains(x, y + this.csm.getCellHeightHalf()) ||
             cellArea.contains(x - this.csm.getCellWidthHalf(), y) ||
             cellArea.contains(x + this.csm.getCellWidthHalf(), y))
         {
            this.paints.drawRobot(g, this.robotLocation, this.robotRotation);
         }
      }
    ...
    */
   @Override
   public void eventFired(MazeCell cell)
   {
      this.invalidateCell(cell);
   }
}
